...
export class RapatQueueComponent implements OnInit, OnDestroy {
  ...
  columns: STColumn[] = [
    {
      ...
      buttons: [
        ...
        },
        {
          ...
          iif: item => {
            if (this.settingService.user.id === item.requestBy.id && moment().isSame(moment(item.tglRequest), 'day')) {
              return true;
            } else {
              return false;
            }
  ...
  schema: SFSchema = {
    ...
    required: ['jenisRequest', 'caseIdReturned'],
    ui: {
      size: 'large',
    },
  };
    ...
    switch (data.tanggapanRequest) {
      case '3111':
        if ('pk' in this.editDataKabid.caseId) {
          if (this.editDataKabid.caseId.pk !== null) return 'Sudah Dijadwalkan Rapat PK';
        }
        return <LogRequestUpdateInput>{
      ...
      case '5111':
        if ('pk' in this.editDataKabid.caseId) {
          if (this.editDataKabid.caseId.pk !== null)
            return 'Sudah ada rapat PK. Rubah keputusan rapat pk menjadi Transfer';
        }
